...
| `VLLM_METAL_DISABLE_GQA_DECODE` | `0` | Emergency override for the GQA-packed decode kernel, which serves the query heads that share a KV head in one threadgroup and reads each KV head once per decode step. It is dispatched only for measured shapes: head_dim 512 with 8 query heads per KV head, once a decode batch's tokens × context reach 4096. Set to `1` to keep every decode batch on the per-token kernel. |
...
| `VLLM_METAL_GEMMA4_VISION_FLOAT32` | `0` | Opt-in Gemma 4 vision precision: the vision sidecar runs the vision tower and the multimodal embedder in float32, and the image processor's float32 `pixel_values` skip vLLM's cast to the model dtype. The language model still receives image rows in its own dtype. Costs a float32 copy of the tower weights (1.1 GB more for the 26B checkpoint; its KV cache budget shrinks by about 1.4 GB) and a slower encoder pass (318 ms instead of 280 ms for a 2 520-patch image on an M3 Ultra, +42 ms time to first token). Set to `1` to enable. |
